(function ($, undefined) {
  ...
  $.extend($.easing, {
    ...
    easeInOutBounce: function (e, t, n, r, i) {
      if (t < i / 2) {
        return $.easing.easeInBounce(e, t * 2, 0, r, i) * 0.5 + n;
      }
  ...
  (b = (function () {
    ...
    (b.prototype.refresh = function () {
      ...
      b.destroy(),
        ...
        b.init(),
        b.changeSlide({ data: { message: "index", index: c } }, !1);
    }),
...
jQuery(document).ready(function ($) {
  //slider height
  if ($(window).width() > 650) {
    var alto_page = $(window).height();
    var alto_cats = $("#logi_categories").outerHeight();

    //alto product
    if ($("#logi_max_container").hasClass("logi_ue")) {
      var alto_cats_fixed = $("#logi_categories_fixed").height();
      if (alto_page - alto_cats_fixed < 500) {
        $(".logi_ue_prod_slide").css(
          "height",
          alto_page - alto_cats_fixed - 40 + "px"
        );
      }
    }
  ...
  //scroll navigation***************************************
  $(document).on("click", ".logi_nav", function (event) {
    event.preventDefault();
    event.stopPropagation();
    var target = $(this).attr("href");
    var finalposition = $(target).offset().top;
    if ($(window).width() > 800) {
      if (
        $("#logi_max_container").hasClass("logi_gamming") ||
        $("#logi_max_container").hasClass("logi_ue") ||
        $("#logi_max_container").hasClass("logi_vc") ||
        $("#logi_max_container").hasClass("logi_blue")
      ) {
        var banner_prev = $(target).prev(".logi_section").height();
        var margin =
          $("#logi_categories_fixed").outerHeight() + banner_prev + 80;
      } else {
        var margin = $("#logi_categories_fixed").outerHeight() + 80;
      }
      //console.log("ooooo--"+margin);
      if (target == "#vc_top_featured") {
        var margin = 0;
      }
    } else {
      var margin = 0;
    ...
    $("html, body").animate(
      { scrollTop: finalposition - margin },
      800,
      "easeOutExpo"
    );
  });
  //fixed menu cats***************************************
  if ($("#logi_categories_fixed").length) {
    if ($(window).width() > 768) {
      //var limitCats = $('#logi_categories').offset().top  ;
      $(window).scroll(function () {
        var scroll = $(window).scrollTop();
        if (scroll > 300) {
          $("#logi_categories_fixed").addClass("fixed");
        } else {
          $("#logi_categories_fixed").removeClass("fixed");
        }
      });
    }
  }

  var arrow_left = "./images/arrow_left_w.png";
  var arrow_right = "./images/arrow_right_w.png";
  var speed = 4000;
  ...
  $("#logi_slider").slick({
    ...
    focusOnSelect: false,
    prevArrow:
      '<div class="flex-prev nw_arrows"><img src="' + arrow_left + '"></<div>',
    nextArrow:
      '<div class="flex-next nw_arrows"><img src="' + arrow_right + '"></<div>',
  });
  var items_to_show = $("#logi_cats_gamming .logi_nav").length;
  if (items_to_show > 4) {
    items_to_show = 4;
  }
  $("#logi_cats_gamming").slick({
    arrows: true,
    infinite: true,
    autoplay: false,
  ...
  /*UE SLIDER PRODUCT*/
  if (
    ($(".logi_ue_product_slidercolors").length ||
      $(".logi_product_slidercolors").length) &&
    $(".logi_colors").length
  ) {
    if ($(".logi_ue_product_slidercolors").length && $(".logi_colors").length) {
      var sliderColor = ".logi_ue_product_slidercolors";
      var colorArrows = true;
    } else {
      var sliderColor = ".logi_product_slidercolors";
      var colorArrows = false;
    }
    $(sliderColor).slick({
      arrows: colorArrows,
      infinite: true,
      ...
      fade: true,
      prevArrow:
        '<div class="flex-prev nw_arrows"><img src="' +
        arrow_left +
        '"></<div>',
      nextArrow:
        '<div class="flex-next nw_arrows"><img src="' +
        arrow_right +
        '"></<div>',
    });
    $(".logi_colors").each(function () {
      var slick = $(this),
        item = $(this).data("nav");
      slick.slick({
        slidesToShow: 6,
  ...
  if ($(".vcslidervideo").length) {
    var slidervideoH = $("#logi_slider").height();
    if (slidervideoH > 0) {
  ...
  $(window).bind("load", function () {
    ...
    //via iframe
    if (clientfull.includes("logitechsouthcone.com")) {
      var client = clientfull.split("?sv=");
      if (clientfull.includes("&")) {
        client = client[1].split("&");
        client = client[0];
      } else {
        client = client[1];
      }
    } else {
      client = document.location.hostname.replace("www.", "");
    }

    if (client != "") {
      $("#logi_max_container").addClass(client);
    }
  });
}); //FIN
